...
export function handleClick_Quotation(rawAbstract, flag) {
  ...
  // Build fully project IOList
  const iolist = Dt.addMandatorySlotTofullIolistProject();
  console.log("addMandatorySlotTofullIolistProject", iolist);
  // Build fully main IOList
  const test2 = Dt.projectIolistFusion(iolist);

  //
  const fullIOlist = Dt.ioListBuilder();
  console.log("ioListBuilder", fullIOlist);
  //
  console.log("fusionIolist", test2);
  //
